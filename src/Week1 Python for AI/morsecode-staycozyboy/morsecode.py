...
def is_validated_morse_code(user_input):
    ...
    # 1번 조건
    for char in user_input:
        if not (char in morsechars):
            return False

    # 2번 조건
    for morse_input in user_input.split():
        if not (morse_input in morse_code_values):
            return False
...
def decoding_character(morse_character):
    ...
    result = morse_keys[morse_values.index(morse_character)]
